"""
通过 LiteLLM Anthropic Passthrough 端点测试 Skill
使用原生 Anthropic API 格式
"""
...
ANTHROPIC_API_KEY = None  # 从环境变量读取或直接设置

SKILL_ID = "skill_015FtmDcs3NUKhwqTgukAyWc"


def test_passthrough_skill():
    """使用 Anthropic passthrough 端点测试 Skill"""

    # 使用 passthrough 端点
    url = f"{LITELLM_BASE_URL}/anthropic/v1/messages"

    # LiteLLM passthrough: Authorization 做 LiteLLM 认证，不传 x-api-key（让 LiteLLM 用服务器配置的）
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {LITELLM_API_KEY}",  # LiteLLM 认证
...
请进行完整分析并给出Go/No-Go建议。"""
    ...
    print("=" * 70)
    print("  🏠 民宿市场调研 Skill 测试 (Anthropic Passthrough)")
    print("=" * 70)
    print(f"\nSkill ID: {SKILL_ID}")
